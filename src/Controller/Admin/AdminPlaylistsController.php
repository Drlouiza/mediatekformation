...
class AdminPlaylistsController extends AbstractController
{
    ...
    public function edit(Playlist $playlist, Request $request): Response
    {
        ...
        if ($formPlaylist->isSubmitted() && $formPlaylist->isValid()){
            ...
            $this->addFlash('success','La modification de la playlist ' . $playlist->getName() . " a été effectuée avec succès."
            );
        ...
        Response
    {
        if(count($playlist->getFormations()) > 0){
            $this->addFlash('alert', 'Impossible de supprimer la playlist ' . $playlist->getName() . ' car elle contient des formations associées.'
            );
            return $this->redirectToRoute('admin.playlists');
        }
